...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orderer-copy.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    ...
    infoln "Creating Transporter Identities peer1"

    createTransporterPeer1
    
    infoln "Creating Distributor Identities peer1"

    createDistributorPeer1

    infoln "Creating Retailer Identities peer1"

    createRetailerPeer1
    
    infoln "Creating Manufacturer Identities peer1"

    createManufacturerPeer1

    infoln "Creating Consumer Identities peer0"

    createConsumer
    
    infoln "Creating Consumer Identities peer1"
...
function createChannel() {
  ...
  scripts/createChannelUpdated.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}


## Call the script to deploy a chaincode to the channel
function deployCC() {
  scripts/deployCCUpdated.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
COMPOSE_FILE_BASE=docker/docker-compose-pharma.yaml
# docker-compose.yaml file if you are using couchdb
COMPOSE_FILE_COUCH=docker/docker-compose-couch-copy.yaml
